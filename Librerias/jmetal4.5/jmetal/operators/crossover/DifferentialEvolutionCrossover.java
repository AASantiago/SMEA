...
public class DifferentialEvolutionCrossover extends Crossover {
  ...
  private static final List VALID_TYPES = Arrays.asList(RealSolutionType.class,
    ...
    if (!(VALID_TYPES.contains(parent[0].getType().getClass()) &&
		...
		if ((DE_Variant_.compareTo("rand/1/bin") == 0) || 
          ...
          */
			...
			for (int j=0; j < numberOfVariables; j++) {
		...
		else if((DE_Variant_.compareTo("de/rand/1") == 0)){
                        for (int j=0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value ;
					value = xCurrent.getValue(j)  + F_ * (xParent0.getValue(j) -
							                                  xParent1.getValue(j));
					if (value < xChild.getLowerBound(j))
						value =  xChild.getLowerBound(j) ;
					if (value > xChild.getUpperBound(j))
                ...
                }else if((DE_Variant_.compareTo("de/rand/2") == 0)){
                        for (int j=0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value ;
					value = xCurrent.getValue(j)  + F_ * (xParent0.getValue(j) -
							                                  xParent1.getValue(j)
                                                +F_*(xParent2.getValue(j)-xParent3.getValue(j)));
					if (value < xChild.getLowerBound(j))
						value =  xChild.getLowerBound(j) ;
					if (value > xChild.getUpperBound(j))
						value = xChild.getUpperBound(j) ;
                ...
                }else if((DE_Variant_.compareTo("de/current-to-rand/2") == 0)){
                        for (int j=0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value ;
					value = xCurrent.getValue(j)  + K_ * (xCurrent.getValue(j) -
							                                  xParent0.getValue(j)
                                                +F_*(xParent1.getValue(j)-xParent2.getValue(j))+F_*(xParent3.getValue(j)-xParent4.getValue(j)));
					if (value < xChild.getLowerBound(j))
						value =  xChild.getLowerBound(j) ;
					if (value > xChild.getUpperBound(j))
						value = xChild.getUpperBound(j) ;
                ...
                }else if((DE_Variant_.compareTo("de/current-to-rand/1") == 0)){
                        for (int j=0; j < numberOfVariables; j++) {
				if (PseudoRandom.randDouble(0, 1) < CR_ || j == jrand) {
					double value ;
					value = xCurrent.getValue(j)  + K_ * (xCurrent.getValue(j) -
							                                  xParent0.getValue(j)
                                                +F_*(xParent1.getValue(j)-xParent2.getValue(j)));
					if (value < xChild.getLowerBound(j))
						value =  xChild.getLowerBound(j) ;
					if (value > xChild.getUpperBound(j))
						value = xChild.getUpperBound(j) ;
