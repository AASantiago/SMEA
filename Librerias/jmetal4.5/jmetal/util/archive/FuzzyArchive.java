 ...
 * defined in NSGA-II).
 */
public class FuzzyArchive extends Archive {
   ...
   */
  public FuzzyArchive(int maxSize, int numberOfObjectives) {
    super(maxSize);
    maxSize_          = maxSize;
  ...
  public boolean add(Solution solution){
    ...
    if (size() > maxSize_) { // The archive is full
      distance_.FuzzyDistanceAssignment(this,objectives_);//dentro de esto esta el problema      
      remove(indexWorst(crowdingDistance_));
      if(solutionsList_.contains(solution))
          return true;//esto es clave me parece!!!!!
    }        
    return false; //aqui hay una diferencia creo
  } // add
} // CrowdingArchive
