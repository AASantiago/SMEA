      ...
      for (j = 0; j < b.length; j++) {
        for (k = 0; k < dim_; k++) {
          ...
          if (k == 0)
            ...
            eps_k = eps_temp; // busca la diferencia más grande en objetivos
        }
        if (j == 0)
          eps_j = eps_k;
        else if (eps_j > eps_k)
          eps_j = eps_k; //busca la diferencia más chica de las grandes
      }
      if (i == 0)
        eps = eps_j;
      else if (eps < eps_j)
        eps = eps_j; //busca la diferencia más grande de las chicas
    }
    return eps;
  } // epsilon
